...
class EmbeddingNN:
    ...
    def __init__(self, heroes_df, embedding_dim):
        ...
        embedding_layer = layers.Embedding(
            len(heroes_df['id']), embedding_dim, input_length=1)
        # Main hero input
        target_hero_input = tf.keras.Input(shape=(1,))
        target_hero_embedding = embedding_layer(target_hero_input)
        target_hero_embedding = layers.Reshape(
            (embedding_dim, 1))(target_hero_embedding)
        ...
        teammate_hero_embedding = layers.Reshape(
            (embedding_dim, 1))(teammate_hero_embedding)
